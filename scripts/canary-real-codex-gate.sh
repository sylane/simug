...
preflight_agent_cmd() {
  ...
  if printf '%s' "$output" | grep -qi "permission denied" && \
    ...
    echo "codex preflight warning: runtime-path maintenance emitted ~/.codex/tmp/arg0/cache warnings, but the probe succeeded so the gate will continue" >&2
  fi
